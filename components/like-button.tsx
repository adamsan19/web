...
const LButton = ({ className, liked, onLike }: any) => {
  return (
    <Button
      className={cn(className, liked ? "bg-red-500 hover:bg-red-600" : "bg-secondary hover:bg-secondary")}
      onClick={onLike}
    >
      {liked ? (
...
const LikeButton = ({
  ...
  if (useButton) {
    return <LButton className={className} liked={liked} onLike={onLike}></LButton>
  }
