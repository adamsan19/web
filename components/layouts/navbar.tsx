  ...
  return (
    <Sheet>
      <SheetTrigger asChild>
        <HamburgerMenuIcon className="size-6" role="menu" aria-label="Open Menu"></HamburgerMenuIcon>
      </SheetTrigger>
      <SheetContent side={"left"}>
        ...
        <div className="grid gap-4 py-4 my-4">
          {folders.map((folder: any) => {
            return (
              <Link href={`/c/${folder.fld_id}`} key={folder.code} className="w-full" role="menuitem">
                <SheetClose className="flex flex-col transition-colors bg-accent text-accent-foreground px-2 py-1 rounded-md w-full">
                  ...
                  <span className="text-[0.65rem] uppercase text-muted-foreground">{folder.total_files} videos</span>
                </SheetClose>
              </Link>
            )
          })}
      ...
      <li>
        <NavigationMenuLink asChild>
          ...
          >
            ...
            <span className="text-[0.65rem] uppercase text-muted-foreground">{videoCount} videos</span>
          </a>
        </NavigationMenuLink>
      </li>
    )
...
const Navbar = async () => {
  ...
  return (
    <div className="flex justify-between items-center px-4 py-3 border-b-[1px]">
      ...
      <div>
        <Link href="/" className="scroll-m-20 text-2xl font-extrabold tracking-tight lg:text-3xl">
          {SITENAME}
        </Link>
      </div>
      <div className="flex flex-row space-x-2">
        <div className="hidden md:flex items-center gap-4">
          <Link href="/liked" className="hover:bg-accent px-2 py-1.5 rounded-md" aria-label="Open liked videos">
            Liked Videos
          </Link>
          <Link href="/admin/settings" className="hover:bg-accent px-2 py-1.5 rounded-md">
            Admin
          </Link>
